...
public class Finger_Formation : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        //stoping avoidence jitter after finger reaches the destonation and there is an obstical near by.
        if (move)
        {
            //determoning if you need to shift left or right to avoid obsticals and then adjusting the p_target accordingly
            p_target = ObsticalDetect();
        }
    ...
    //detecs an obstical is in the way and returns the shifted positon that you need to move twards to avoid it
    private Vector3 ObsticalDetect()
    {
        //raycasts to see if there is an obstical in this fingers way
        RaycastHit hit;
        ...
        for (int i = 0; i < offset.Length; i++)
        {
            ...
            if (Physics.Raycast(sightRay, out hit, sightRange / sightReduction) && hit.collider.CompareTag("Obstical"))
            //if (Physics.Raycast(sightRay, out hit, sightRange) && hit.collider.CompareTag("Obstical"))
            {
                ...
                //Debug.Log("see an obstical");
            }
        }
        //determoning if target should be shifted left or right
        if(obsticalLeft >= obsticalRight && obsticalLeft != 0)
        {
            //rotating the movment direction to the right 
            //moveDirection = Quaternion.Euler(new Vector3(0f, 20f*obsticalLeft, 0f)) * moveDirection;
        ...
        else if(obsticalRight > obsticalLeft)
        {
            //rotating the movement direction to the left
            //moveDirection = Quaternion.Euler(new Vector3(0f, -20f*obsticalRight, 0f)) * moveDirection;
